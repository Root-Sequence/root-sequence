...
This document is the **entire factual evidence packet** for the T-003 editorial pilot.
...
## Follow-up actions stated in the postmortem

Cloudflare identified at least these follow-ups:

### Rate-limiting architecture

The DDoS module was using a **legacy** rate-limiting implementation, while customer-configured rate limits used a newer engine.

Cloudflare stated it was in early implementation stages of replacing the affected service.

The replacement design was intended to allow limits on both uninterrupted and total execution time for a single request.

### Loop containment

Cloudflare said it was exploring ways to prevent the affected service from looping indefinitely through tail calls.

### Staging / rollout

The new DDoS rule had first been staged in a small number of production data centers and then deployed to all data centers a few hours later.

Cloudflare said it would continue improving staging and rollout procedures to reduce change-related blast radius.

### Backbone congestion

For the separate network-congestion event, Cloudflare said it was already:

- expanding backbone capacity in affected data centers;
- improving network mitigation decisions to use more information about available capacity on alternative paths.
